...
class Order_Screen(Column):
    ...
    ORDER_BUTTON_TEXT: dict = {"English": "Order", "Portuguese": "Encomendar"}
    CONFIRM_BUTTON_TEXT: dict = {"English": "Confirm", "Portuguese": "Confirmar"}
    EDIT_BUTTON_TEXT: dict = {"English": "Edit", "Portuguese": "Editar"}
    EDIT_TEXTFIELD_TEXT: dict = {"English": "Quantity", "Portuguese": "Quantidade"}
    EDIT_TEXTFIELD_HINT_TEXT: dict = {"English": "e.g.: 23", "Portuguese": "ex.: 23"}
    ...
    ALERT_DIALOG_OK_TEXT: dict = {"English": "OK", "Portuguese": "OK"}
    ALERT_DIALOG_CANCEL_TEXT: dict = {"English": "CANCEL", "Portuguese": "CANCELAR"}
    ...
    WEEK_DAYS: dict = {
        "Mon": {"English": "Mon", "Portuguese": "Seg"},
        "Tue": {"English": "Tue", "Portuguese": "Ter"},
        "Wed": {"English": "Wed", "Portuguese": "Qua"},
        "Thu": {"English": "Thu", "Portuguese": "Qui"},
        "Fri": {"English": "Fri", "Portuguese": "Sex"},
        "Sat": {"English": "Sat", "Portuguese": "Sab"},
        "Sun": {"English": "Sun", "Portuguese": "Dom"},
    }
    ...
    __current_order: dict = {"date": str, "products": dict}
    ...
    __current_week_text: Text = Text(
        size=15, width=FontWeight.BOLD, color=MAIN_TEXT_COLOR
    )
    ...
    __pass_week_section: Container = Container(alignment=alignment.center)
    ...
    __products_column: Column = Column(
        alignment=MainAxisAlignment.START, scroll=ScrollMode.ADAPTIVE, expand=True
    )

    ###############################
    # Initializing main button section
    __main_button_row: Row = Row(alignment=MainAxisAlignment.CENTER)
    __order_button: Container = Container(
        ...
        border_radius=20,
        scale=1.6,
        animate_opacity=Animation(duration=200, curve=AnimationCurve.EASE_IN_OUT),
    )
    ...
    __alert = AlertDialog(
        ...
        title=Text(color=MAIN_TEXT_COLOR),
        content=Text(color=MAIN_TEXT_COLOR),
    ...
    def __init__(self, page: Page):
        ...
        self.__pass_week_forward_button = IconButton(
            ...
            data="forward",
            style=ButtonStyle(
                padding=padding.all(0), visual_density=VisualDensity.COMPACT
            ),
        )
        ...
        self.__current_week_text.value = (
        ...
        self.__alert.actions = [
            Container(
                content=ElevatedButton(
                    ...
                    bgcolor="transparent",
                    color="#606060",
                    style=ButtonStyle(elevation=0, overlay_color=BUTTON_OVERLAY_COLOR),
                ),
                gradient=Primary_Gradient(),
                border_radius=20,
            ),
    ...
    def refresh_data(self, update_days_row: bool):
        """
        Refresh data about the days and products by full filling the rows and columns info again.
        """
        ...
        if TESTING:
            ...
            for i in range(20):
                self.__current_order["products"][f"Este é um Pao disto assim {i}"] = {
                    "product_id": f"{i}",
                    "quantity_text": Text(
                        value="0", color=MAIN_TEXT_COLOR, weight=FontWeight.BOLD
                    ),
                    "quantity": 0,
                    "cost": 1.50,
                }
        ...
        if self.__refresh_catalog():
            if self.__refresh_week_catalog(monday.strftime("%d-%m-%Y")):
                ...
                for product_id in self.__catalog.keys():
                    self.__current_order["products"][
                        self.__catalog[product_id]["product_title"]
                    ] = {
    ...
    def __edit_current_day(self, e):
        ...
        if TESTING:
            if self.__current_week_day == "Wed" and self.__current_date == "04-12-2024":
                ...
                self.__current_week_day == "Thu" and self.__current_date == "05-12-2024"
            ):
                num = 5
            elif (
                self.__current_week_day == "Fri" and self.__current_date == "06-12-2024"
            ):
                num = 2
            ...
            state = False
            for i in range(num):
                new_container = self.__create_new_product_container(
                    f"Este é um Pao disto assim {i}", i, str(i + 20), state
                )
        ...
        for product_id in self.__catalog.keys():
            ...
            if (
                product_id
                in self.__current_week_catalog[self.__current_week_day].keys()
            ):
                quantity = self.__current_week_catalog[self.__current_week_day][
                    product_id
                ]["product_quantity"]
                state = True
    ...
    def __update_current_day(self, e):
        ...
        new_date_catalog = {"catalog_date": self.__current_date, "catalog_products": {}}
        for product_id in self.__current_date_catalog_edit.keys():
            if self.__current_date_catalog_edit[product_id]["state"]:
                new_date_catalog["catalog_products"][product_id] = {
                    "product_quantity_total": self.__current_date_catalog_edit[
                        product_id
                    ]["quantity"]
                }

        headers = {"Authorization": f"{user_data["token"]}"}

        url_template = Template(endpoints_urls["EDIT_CURRENT_DAY"])
        get_catalog_url = url_template.safe_substitute(
            business_id=shared_vars["current_business"]["id"]
        )
        ...
        try:
            ...
            response = requests.post(
                get_catalog_url, headers=headers, json=new_date_catalog
            )
            ...
            if response.status_code == STATUS_CODES["SUCCESS"]:
                present_snack_bar(
                    self.__page,
                    self.DATE_CATALOG_EDIT_SUCCESS_TEXT[configs["LANGUAGE"]],
                    "Green",
                )

            elif response.status_code >= STATUS_CODES["INTERNAL_ERROR"]:
                present_snack_bar(
                    self.__page, self.INTERNAL_ERROR_TEXT[configs["LANGUAGE"]], "Red"
                )
            else:
                present_snack_bar(
                    self.__page,
                    self.UNRECOGNIZED_ERROR_TEXT[configs["LANGUAGE"]],
                    "Red",
                )
        except requests.exceptions.RequestException as e:
            # Handle network-related errors
            present_snack_bar(
                self.__page, self.NETWORK_ERROR_TEXT[configs["LANGUAGE"]], "Red"
            )
    ...
    def __refresh_week_catalog(self, monday_date: str):
        """
        Refreshes the week catalog for the actual business and save it in self.__current_week_catalog
        If any error occurs return false, otherwise return true
    ...
    def __fill_days_row(self, monday: datetime):
        """
        ...
        self.__create_new_date_button(
            "Tue", (monday + timedelta(days=1)).strftime("%d-%m-%Y")
        )
        self.__create_new_date_button(
            "Wed", (monday + timedelta(days=2)).strftime("%d-%m-%Y")
        )
        self.__create_new_date_button(
            "Thu", (monday + timedelta(days=3)).strftime("%d-%m-%Y")
        )
        self.__create_new_date_button(
            "Fri", (monday + timedelta(days=4)).strftime("%d-%m-%Y")
        )
        self.__create_new_date_button(
            "Sat", (monday + timedelta(days=5)).strftime("%d-%m-%Y")
        )
        self.__create_new_date_button(
            "Sun", (monday + timedelta(days=6)).strftime("%d-%m-%Y")
        )
    ...
    def __fill_products_column(self):
        ...
        if TESTING:
            ...
            for i in range(num):
                if user_data["is_admin"]:
                    product_row = self.__create_new_product_row_manager(
                        f"Este é um Pao disto assim {i}",
                        self.__current_order["products"][
                            f"Este é um Pao disto assim {i}"
                        ]["cost"],
                        i + 10,
                    ...
                    product_row = self.__create_new_product_row_client(
                        "1",
                        f"Este é um Pao disto assim {i}",
                        self.__current_order["products"][
                            f"Este é um Pao disto assim {i}"
        ...
        for product_id in self.__current_week_catalog[self.__current_week_day].keys():
            if user_data["is_admin"]:
                product_quantity_sold = self.__current_week_catalog[
                    self.__current_week_day
                ][product_id]["product_quantity_sold"]
                product_quantity_total = self.__current_week_catalog[
                    self.__current_week_day
                ][product_id]["product_quantity_total"]
                product_row = self.__create_new_product_row_manager(
                    self.__catalog[product_id]["product_title"],
                    self.__catalog[product_id]["product_price"],
                    product_quantity_sold,
                    product_quantity_total,
                )
            else:
                product_scarcity = self.__current_week_catalog[self.__current_week_day][
                    product_id
                ]["product_scarcity"]
                product_row = self.__create_new_product_row_client(
                    product_id,
    ...
    def __change_date_product_list(self, e):
        ...
        if e.control.data[1] != self.__current_date:
            if self.__total_amount > 0:
                ...
                new_current_date_datetime = datetime.strptime(
                    self.__current_date, "%d-%m-%Y"
                )
    ...
    def __change_product_amount(self, e):
        ...
        if operation == "+":
            ...
            if product_scarcity is not None:
                if (
                    self.__current_order["products"][product_name]["quantity"]
                    >= product_scarcity
                ):
                    increment = False
            if increment:
        ...
        self.__current_order["products"][product_name][
            "quantity_text"
        ].value = f"{self.__current_order["products"][product_name]["quantity"]}"
        self.__page.update()
    ...
    def __edit_product_amount(self, e):
        ...
        if e.control.value == "":
            ...
            self.__current_date_catalog_edit[product_id]["quantity"] = int(
                e.control.value
            )
    ...
    def __create_new_date_button(self, week_day: str, date: str):
        """
                    ...
                    Container(
                        content=Row(
                            controls=[
                                Container(
                                    content=Text(
                                        value=f"{product_name}", color=MAIN_TEXT_COLOR
                                    ),
                                    padding=padding.only(left=5, right=5),
                                    expand=True,
                                ),
    ...
    def __create_new_product_container(
        ...
        return Container(
            content=Container(
                content=Column(
                    controls=[
                        Text(value=f"{product_name}", color=MAIN_TEXT_COLOR),
                        Row(
                            controls=[
                                Checkbox(
                                    adaptive=True,
                            ...
                            content=Column(
                                controls=[
                                    ...
                                    Container(
                                        ...
                                        padding=Padding(
                                            left=8, top=1, right=8, bottom=10
                                        ),
                                        alignment=alignment.center,
                                    ),
                                ],
